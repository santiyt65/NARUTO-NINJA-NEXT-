...
mobile: MethodMobile, 
browser: opcion == '1' ? ['TheMystic-Bot-MD', 'Safari', '2.0.0'] : methodCodeQR ? ['GOGETA-BOT', 'Safari', '2.0.0'] : ['Ubuntu', 'Chrome', '20.0.04'],
auth: {
creds: state.creds,
...
let prekey = []
let directorio = readdirSync("./GogetaSession")
let filesFolderPreKeys = directorio.filter(file => {
return file.startsWith('pre-key-') /*|| file.startsWith('session-') || file.startsWith('sender-') || file.startsWith('app-') */
})
prekey = [...prekey, ...filesFolderPreKeys]
filesFolderPreKeys.forEach(files => {
unlinkSync(`./GogetaSession/${files}`)
})
} 
...
const directories = ['./GogetaSession/', './jadibts/']
...
if (reason == 405) {
await fs.unlinkSync("./GogetaSession/" + "creds.json")
